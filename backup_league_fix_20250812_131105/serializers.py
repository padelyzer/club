...
# League Scheduling Serializers
class LeagueSerializer(serializers.ModelSerializer):
    """
    Serializer for league list view.
    """
    ...
    class Meta:
        ...
        model = League
        fields = [
            'id', 'name', 'description', 'format', 'format_display',
            'frequency', 'frequency_display', 'start_date', 'end_date',
            'divisions', 'is_active', 'teams_count', 'matches_count',
            'created_at'
        ]
...
class LeagueDetailSerializer(LeagueSerializer):
    ...
    class Meta(LeagueSerializer.Meta):
        fields = LeagueSerializer.Meta.fields + [
            'club_name', 'matches_per_pair', 'preferred_start_hour',
            'preferred_end_hour', 'weekend_play', 'weekday_play',
            'home_away', 'max_travel_distance', 'registration_fee',
            'constraints_count', 'latest_schedule'
        ]
    
    def get_constraints_count(self, obj):
        return obj.constraints.filter(is_active=True).count()
    
    def get_latest_schedule(self, obj):
        latest = obj.schedules.order_by('-generation_timestamp').first()
        if latest:
            return {
                'id': latest.id,
...
class LeagueTeamSerializer(serializers.ModelSerializer):
    """
    Serializer for league teams.
    """
    
    player1 = ClientProfileSerializer(read_only=True)
    player2 = ClientProfileSerializer(read_only=True)
    home_venue_name = serializers.CharField(source='home_venue.name', read_only=True)
    
    class Meta:
        from .models import LeagueTeam
        model = LeagueTeam
        fields = [
            'id', 'name', 'league', 'player1', 'player2', 'division',
            'home_venue', 'home_venue_name', 'location_lat', 'location_lng',
            'availability_preferences', 'current_points', 'matches_played',
            'matches_won', 'created_at'
        ]


class LeagueTeamCreateSerializer(serializers.ModelSerializer):
    """
    Serializer for creating league teams.
    """
    ...
    class Meta:
        ...
        model = LeagueTeam
        fields = [
            'league', 'name', 'player1', 'player2', 'division',
            'home_venue', 'location_lat', 'location_lng',
    ...
    def validate(self, data):
        ...
        league = data['league']
        from .models import LeagueTeam
        if LeagueTeam.objects.filter(league=league, name=data['name']).exists():
            raise serializers.ValidationError("Team name already exists in this league")
...
class LeagueMatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        model = LeagueMatch
        fields = [
            'id', 'league', 'round_number', 'match_number',
            'home_team', 'home_team_name', 'away_team', 'away_team_name',
            'scheduled_datetime', 'court', 'court_name', 'status',
            'status_display', 'home_score', 'away_score', 'winner',
            'winner_name', 'travel_distance', 'scheduling_priority',
            'created_at'
        ]
...
class LeagueScheduleSerializer(serializers.ModelSerializer):
    """
    Serializer for league schedules.
    """
    ...
    class Meta:
        from .models import LeagueSchedule
        model = LeagueSchedule
        fields = [
            'id', 'league', 'league_name', 'round_number', 'start_date',
            'end_date', 'generation_timestamp', 'algorithm_used',
            'quality_score', 'schedule_data', 'status', 'status_display',
            'matches_count'
        ]
    
    def get_matches_count(self, obj):
        return obj.league.matches.count()


class ScheduleConstraintSerializer(serializers.ModelSerializer):
    """
    Serializer for schedule constraints.
    """
    ...
    class Meta:
        ...
        fields = [
            'id', 'league', 'constraint_type', 'constraint_type_display',
            'priority', 'priority_display', 'parameters', 'is_active',
            'description', 'created_at'
        ]


class ScheduleConstraintCreateSerializer(serializers.ModelSerializer):
    """
    Serializer for creating schedule constraints.
    """
    ...
    class Meta:
        ...
        model = ScheduleConstraint
        fields = [
            'league', 'constraint_type', 'priority', 'parameters',
            'description', 'is_active'
        ]
    
    def validate_parameters(self, value):
        """Validate constraint parameters based on type."""
        constraint_type = self.initial_data.get('constraint_type')
        
        if constraint_type == 'blackout_dates':
            if 'dates' not in value:
                raise serializers.ValidationError(
                    "Blackout dates constraint requires 'dates' parameter"
                )
            if not isinstance(value['dates'], list):
                raise serializers.ValidationError(
                    "'dates' parameter must be a list"
                )
        
        elif constraint_type == 'time_preference':
            if 'preferred_hours' not in value:
                raise serializers.ValidationError(
                    "Time preference constraint requires 'preferred_hours' parameter"
                )
        
        elif constraint_type == 'travel_distance':
            if 'max_distance_km' not in value:
                raise serializers.ValidationError(
                    "Travel distance constraint requires 'max_distance_km' parameter"
                )
            if not isinstance(value['max_distance_km'], (int, float)):
                raise serializers.ValidationError(
                    "'max_distance_km' must be a number"
                )
...
class ScheduleOptimizationSerializer(serializers.ModelSerializer):
    """
    Serializer for schedule optimization runs.
    """
    ...
    class Meta:
        from .models import ScheduleOptimization
        model = ScheduleOptimization
        fields = [
            'id', 'league', 'algorithm', 'algorithm_display',
            'start_time', 'end_time', 'duration_seconds', 'quality_score',
            'travel_burden_score', 'time_slot_utilization', 'fairness_index',
            'conflict_count', 'parameters', 'results_data', 'status',
            'status_display', 'error_message'
        ]
